...
import  { postSignin } from '../../config/reducers/auth/signinSlice';
...
const SignIn: React.FC = () => {
  ...
  const dispatch = useDispatch<AppDispatch>(); // menambahkan AppDispatch supaya type data dispatch bisa di gunakan atau terbaca 
  const user = useSelector((state: RootState) => state.authSlice.signin) // menambahkan RootState supaya type data state bisa di gunakan atau terbaca
...
useEffect(() => {
  if (user.status == "success" || user.status == "error")
    console.log(user); 
  },[user]); //[user] untuk mendeteksi perubahan pada user, ketika ada perubahan maka useEffect jalan 
